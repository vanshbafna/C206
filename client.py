...
def rightBoard():
    ...
    for box in range(0,11):
        if(box == 10):
            ...
            #boxLabel.config(height=3, width=10) 
            boxLabel.place(x=xPos, y=screen_height/2 - 100)
...
def gameWindow():
    ...
    img_old=Image.open("/Users/vanshbafna/Desktop/module5/C206/assets/background.png")
    img_resized=img_old.resize((screen_width, screen_height)) # new width & height
    ...
    # Display image
    canvas2.create_image( 0, 0, image = my_img, anchor = "nw")
...
def askPlayerName():
    ...
    nameEntry.place(x = screen_width/2 - 260, y=screen_height/4 + 100)

    button = Button(nameWindow, text="Save", font=("Chalkboard SE", 13),width=15,  height=2, bg="#80deea", bd=3, command=saveName,)
    button.place(x = screen_width/2 - 40, y=screen_height/2 + 20 )
